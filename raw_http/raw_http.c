...
void
parse_line (char *line, HttpRequest *request, int *is_body)
{
  if (strcmp (line, "\r\n") == 0 || strcmp (line, "\n") == 0)
    {
      *is_body = 1;
...
struct curl_slist *
configure_http_headers (HttpRequest *request)
{
...
size_t
header_callback (void *ptr, size_t size, size_t nmemb, HttpResponse *response)
{
...
size_t
write_callback (void *ptr, size_t size, size_t nmemb, HttpResponse *response)
{
...
main (int argc, char *argv[])
{
  ...
  while ((read = getline (&line, &len, input_file)) != -1)
    {
      parse_line (line, &request, &is_body);
    }
  ...
  if (res != CURLE_OK)
  ...
  else
    {
      ...
      if (output_file)
        ...
        {
          fprintf (stderr,
                   "\033[1;31mError: Could not open output file %s\033[0m\n",
                   output_filename);
        }
    }
